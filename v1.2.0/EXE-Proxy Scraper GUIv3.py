...
import requests
from proxies import get_proxy, get_proxy22
from bs4 import BeautifulSoup
import os
from random import choice
from selenium_scraper import SpyProxies, get_proxy3, get_proxy4
...
class ScrapeTH(Thread):
    ...
    def run(self):
        self.threadsl.append(Thread(target=get_proxy, args=[self.result1, self.senal]))
        self.threadsl.append(Thread(target=self.get_prox, args=[self.result1, self.senal]))
        self.threadsl.append(Thread(target=SpyProxies, args=[self.senal]))
        self.threadsl.append(Thread(target=get_proxy4, args=[self.senal]))
        self.threadsl.append(Thread(target=get_proxy3, args=[self.result1, self.senal]))
        self.threadsl.append(Thread(target=get_proxy22, args=[self.result1, self.senal]))
    ...
    def get_prox(self, listn, senal):
        ...
        for proxy in list2:
            if proxy.count(".") == 3 and proxy.count(":") == 1 and proxy[0].isnumeric():
                senal.emit(proxy + "\n")
        return list2
...
class MiVentana(QtCore.QObject):
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.setFixedSize(800, 548)
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("EXE-Proxy Scraper by rivem0dz", "EXE-Proxy Scraper by rivem0dz"))
    ...
    def scrapear(self, evento):
        if evento != "END":
            ...
            self.textBrowser.setText(evento + self.textBrowser.toPlainText()) 
        else:
            print("END")
